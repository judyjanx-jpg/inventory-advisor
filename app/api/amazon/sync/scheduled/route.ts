  ...
  try {
    do {
      ...
      for (const order of orders) {
        ...
        // Check if order exists and what its previous status was
        const existingOrder = await prisma.order.findUnique({
          where: { id: orderId },
          select: { status: true },
        })
        
        const newStatus = order.OrderStatus || 'Unknown'
        const wasPending = existingOrder?.status === 'Pending'
        const isNowShipped = ['Shipped', 'Unshipped', 'PartiallyShipped'].includes(newStatus)
        const statusChangedFromPending = wasPending && isNowShipped
        ...
        // Always get order items, but especially important if status changed from Pending
        // This ensures we get updated pricing when orders ship
        await delay(200)
...
/**
 * Re-sync orders that were previously Pending and now have pricing available
 * This fixes orders that were synced with $0 item prices before Amazon finalized them
 */
async function resyncPendingOrders(client: any, marketplaceId: string, daysBack: number = 7) {
  console.log(`\n🔄 Re-syncing previously Pending orders...`)
  ...
  try {
    ...
    if (ordersToResync.length === 0) {
      console.log(`  ✓ No orders need re-syncing`)
      return { resynced: 0, itemsUpdated: 0 }
    }
    ...
    console.log(`  ⚠️ Re-sync error: ${error.message}`)
  }
  
  console.log(`  ✓ Re-synced ${resynced} orders, updated ${itemsUpdated} items`)
  return { resynced, itemsUpdated }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Re-sync previously Pending orders that now have pricing (fixes $0 item prices)
    results.pendingResync = await resyncPendingOrders(client, credentials.marketplaceId, config.daysBack)
