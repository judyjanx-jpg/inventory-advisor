...
export default function AlertCenter() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div
          className={`bg-[var(--card)] rounded-xl p-4 border cursor-pointer transition-all ${
            urgencyFilter === 'CRITICAL' ? 'border-red-500 ring-1 ring-red-500' : 'border-[var(--border)] hover:border-red-500/50'
          }`}
        ...
        <div
          className={`bg-[var(--card)] rounded-xl p-4 border cursor-pointer transition-all ${
            urgencyFilter === 'HIGH' ? 'border-orange-500 ring-1 ring-orange-500' : 'border-[var(--border)] hover:border-orange-500/50'
          }`}
          onClick={() => setUrgencyFilter(urgencyFilter === 'HIGH' ? 'all' : 'HIGH')}
        >
          <div className="flex items-center justify-between">
            <p className="text-sm text-[var(--muted-foreground)]">High</p>
        ...
        <div
          className={`bg-[var(--card)] rounded-xl p-4 border cursor-pointer transition-all ${
            urgencyFilter === 'MEDIUM' ? 'border-yellow-500 ring-1 ring-yellow-500' : 'border-[var(--border)] hover:border-yellow-500/50'
          }`}
        ...
        <div
          className={`bg-[var(--card)] rounded-xl p-4 border cursor-pointer transition-all ${
            urgencyFilter === 'LOW' ? 'border-blue-500 ring-1 ring-blue-500' : 'border-[var(--border)] hover:border-blue-500/50'
          }`}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="flex bg-[var(--card)] rounded-lg border border-[var(--border)] p-1">
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm transition-colors ${
                filter === 'unacknowledged' ? 'bg-cyan-600 text-[var(--foreground)]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm transition-colors ${
                filter === 'acknowledged' ? 'bg-cyan-600 text-[var(--foreground)]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'
              }`}
            >
              Acknowledged
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm transition-colors ${
                filter === 'all' ? 'bg-cyan-600 text-[var(--foreground)]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'
              }`}
            >
              All
            </button>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 bg-[var(--card)] border border-[var(--border)] rounded-lg text-sm ${
              showFilters ? 'text-cyan-400' : 'text-[var(--muted-foreground)]'
        ...
        <div className="flex items-center gap-2">
          ...
          {alerts.filter(a => !a.acknowledgedAt).length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-cyan-600 hover:bg-cyan-700 rounded-lg text-sm text-[var(--foreground)]"
            >
      ...
      {/* Extended Filters */}
      {showFilters && (
        <div className="flex items-center gap-4 p-4 bg-[var(--card)] rounded-lg border border-[var(--border)]">
          <div>
            <label className="text-xs text-[var(--muted-foreground)] block mb-1">Alert Type</label>
            <select
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value)}
              className="bg-[var(--secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-[var(--foreground)] text-sm"
            >
      ...
      <div className="space-y-3">
        {filteredAlerts.length === 0 ? (
          <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-8 text-center">
            ...
            <p className="text-[var(--muted-foreground)]">No alerts match your filters</p>
          </div>
        ) : (
          filteredAlerts.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className={`bg-[var(--card)] rounded-xl border p-4 transition-all ${
                  alert.acknowledgedAt ? 'border-[var(--border)] opacity-60' : config.borderColor
                }`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="font-medium text-[var(--foreground)]">{alert.title}</h4>
                  ...
                  <div className="flex items-center gap-2">
                    {!alert.acknowledgedAt && (
                      <button
                        ...
                        className="p-2 hover:bg-[var(--muted)] rounded-lg text-[var(--muted-foreground)] hover:text-green-400"
                        title="Acknowledge"
                      >
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-[var(--muted)] rounded-lg text-[var(--muted-foreground)] hover:text-cyan-400"
                      title="Mark Resolved"
                    >
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-[var(--muted)] rounded-lg text-[var(--muted-foreground)] hover:text-red-400"
                      title="Dismiss"
                    >
                      <X className="w-5 h-5" />
                    </button>
                ...
                {alert.metadata && Object.keys(alert.metadata).length > 0 && (
                  <div className="mt-3 pt-3 border-t border-[var(--border)] flex items-center gap-4 text-sm">
                    {Object.entries(alert.metadata).map(([key, value]) => (
                      <span key={key} className="text-[var(--muted-foreground)]">
